...
const seedGovernmentServicesClean = async () => {
  try {
    ...
    // Services to insert (all use 'consumer' service type only)
    const services = [
      {
    ...
    services.forEach((service, index) => {
      console.log(`${index + 1}. ${service.name} (${service.category}) - Rs.${service.base_price}`);
    });
